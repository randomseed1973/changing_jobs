def c19_w_confirmed():
    ...
    c19w_df = pd.read_csv('./Original_Data/'+ "time_series_covid19_confirmed_global.csv" , encoding = "utf=8")
    ...
    #同じ名前が複数ある国の確認
    print(c19w_df["Country/Region"].value_counts().head(10))
    ...
    for St in cc188:
        ...
        St_day.columns = Cn
        # print(St_ed_df.head())
        St_day.to_csv("./csv/csv_day_Confirmed/" + St + "_day.csv", encoding="utf-8", index=None)
        #print(St_day)
        St_d.clear()
        St_ED.clear()
    ...
    c19w_Confirmed = pd.concat(lo_df , sort=False)

    #データフレームc19w_Recoveredに新しい列Date_new行を追加
    c19w_Confirmed["Date_new"] =  pd.DataFrame(ymd_l)

    #不要になった列Dateの削除
    del c19w_Confirmed["Date"]

    #カラムの並び替え
    c19w_Confirmed= c19w_Confirmed[["Date_new","Country","Confirmed"]]

    #カラムのリネーム
    c19w_Confirmed = c19w_Confirmed.rename(columns={"Date_new" : "Date"})

    #csvファイルへの書き込み
    c19w_Confirmed.to_csv("./csv/csv_comp/" + "c19w_Confirmed.csv",index =None , encoding = "utf=8")

    #csvファイルからの読み込み
    c19w_Confirmed = pd.read_csv("./csv/csv_comp/" + "c19w_Confirmed.csv" , encoding = "utf=8")
